...
function parse_args(args)
   ...
   local ring_buffer_size, batch_pull_size
   ...
   function handlers.k (arg)
	   batch_pull_size = tonumber(arg)
	   if not batch_pull_size then fatal("bad batch size: " .. arg) end
	   if batch_pull_size > (require('core.link').max + 1) then
		   fatal("Batch pull size cannot be greater than link buffer size: " .. batch_pull_size)
	   end
   end
   ...
   lib.dogetopt(args, handlers, "b:c:n:m:vD:hir:k:",
      { conf = "c", ["v4-pci"] = "n", ["v6-pci"] = "m",
        verbose = "v", duration = "D", help = "h",
        virtio = "i", ["ring-buffer-size"] = "r",["batch-size"] = "k", cpu = 1,
        ["real-time"] = 0, ["no-ingress-drop-monitor"] = 0, })

   if batch_pull_size ~= nil then
	   require('apps.intel.intel_app').pull_size = batch_pull_size
   end
